    ...
    def run(self):
        ...
        for episode in range(NUM_EPISODES):
            ...
            for step in range(MAX_STEPS):
                ...
                print(action[0][0], type(action[0][0]))
                print(action.item(), type(action.item()))
                am, field, step_reward, done = self.env.step(int(action.item()))
                ...
                if done:
                    print(f"{episode} Episode: Finished with score {sum_reward} : 10試行の平均SCORE = {episode_10_list.mean():.1f}")
                    break
